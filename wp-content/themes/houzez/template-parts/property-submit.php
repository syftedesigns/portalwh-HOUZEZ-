...
// Editado aqui Carlos Estarita
    if ($enable_paid_submission == 'membership' && $remaining_listings != -1 && $remaining_listings < 1 && is_user_logged_in() ) {
        if (!houzez_user_has_membership($userID)) {
            // Carlos estarita, aca vamos a colocar el mismo codigo del else para evitar que el sistema no me bloquee la carga de propeidades si se acaba de registrar
            // En caso de eliminar, por favor remover el código duplicado que colocare a continuación y eliminar el documentado
            ...
            ?>
            <!-- DUPLICADO, ELIMINAR SI QUIERO PAGAR UN PAQUETE POR POSTEAR ANUNCIOS -->
            ...
            if(houzez_option('add-prop-gdpr-enabled')) {
                get_template_part('template-parts/submit-property/gdpr'); 
            }?>
            ...
            <div class="steps-nav">
                ...
                <div class="btn-right action btn-submit btn-step-submit">
                    <span><?php esc_html_e('Submit Property', 'houzez'); ?></span> 
                    <button id="add_new_property" type="submit" class="btn"><i class="fa fa-angle-right"></i></button>
                </div>
            </div>
            </div>
        </form>
