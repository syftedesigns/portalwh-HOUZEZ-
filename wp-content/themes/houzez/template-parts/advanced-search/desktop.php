...
if (isset($_GET['search_location'])) {
    $search_location = esc_attr($_GET['search_location']);
}

if (isset($_GET['min-area'])) {
    $min_area = esc_attr($_GET['min-area']);
}

if (isset($_GET['max-area'])) {
    $max_area = esc_attr($_GET['max-area']);
}
        ...
        <div class="row">
            <div class="col-sm-12">
                <form method="get" autocomplete="off" action="<?php echo esc_url( $search_template ); ?>">
                                ...
                                <div class="col-md-4 col-sm-5">
                                    <div class="form-group no-margin">
                                        <div class="input-search input-icon">
                                            <input type="text" id="keyword" class="form-control" value="<?php echo esc_attr($keyword); ?>" name="keyword" placeholder="<?php echo $keyword_field_placeholder; ?>">
                                ...
                                <div class="<?php echo esc_attr($geo_location_field_classes); ?>">
                                    <div class="form-group no-margin">
                                        <div class="search-location">
                                            <input type="text" id="location_search_main" class="form-control search_location" value="<?php echo esc_attr($search_location); ?>" name="search_location" placeholder="<?php echo esc_html__('Location', 'houzez'); ?>">
                    ...
                    <?php if( $enable_radius_search == 1 ) { ?>
                            <input type="hidden" name="lat" value="<?php echo isset ( $_GET['lat'] ) ? esc_attr($_GET['lat']) : ''; ?>" id="latitude">
                            <input type="hidden" name="lng" value="<?php echo isset ( $_GET['lng'] ) ? esc_attr($_GET['lng']) : ''; ?>" id="longitude">
                            <input type="checkbox" name="use_radius" id="use_radius" <?php checked( true, $checked ); ?>>
                            <div class="form-group search-long">
                                <div class="search">
                                    <?php if ($adv_show_hide['keyword'] != 1) { ?>
                                    <div class="input-search input-icon">
                                        <input class="form-control" type="text" value="<?php echo sanitize_text_field($keyword); ?>" name="keyword" placeholder="<?php echo $keyword_field_placeholder; ?>">
                                        <div id="auto_complete_ajax" class="auto-complete"></div>
                                    </div>
                                    <?php } ?>
                                    <div class="search-location">
                                        <input type="text" class="form-control search_location" value="<?php echo sanitize_text_field($search_location); ?>" name="search_location" placeholder="<?php echo esc_html__('Location', 'houzez'); ?>">
                                        <i class="location-trigger fa fa-dot-circle-o"></i>
                                    </div>
                                    <select name="radius" class="selectpicker" data-live-search="true" data-live-search-style="begins">
                                        <option value="0"><?php esc_html_e('Radius','houzez');?></option>
                                            ...
                                            if( $is_search == 'yes' ) { 
                                                ...
                                                if($adv_show_hide[$field_name] != 1 ) {
                                                    if($field_type == 'select') { ?>
                                                        ...
                                                        <div class="col-sm-3 col-xs-6">
                                                            <div class="form-group">
                                                                <select name="<?php echo esc_attr($field_name);?>" class="selectpicker <?php echo esc_attr($field_name);?>" data-live-search="false" data-live-search-style="begins" title="<?php echo $field_title; ?>">
                                    ...
                                    if(class_exists('Houzez_Currencies')) {
                                        ...
                                        if($currencies) {
                                            ...
                                            foreach ($currencies as $currency) {
                                                echo '<option '.selected( $currency->currency_code, $searched_currency, false).' value="'.$currency->currency_code.'">'.esc_attr($currency->currency_code).'</option>'; 
                                            }
                                            echo '</select>';
                                            echo '</div>';
                                            echo '</div>';
