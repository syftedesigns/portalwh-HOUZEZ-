...
// Carlos Estarita, Aqui update
?>
<!--<div class="validate-errors-gal alert alert-success alert-dismissible" role="alert" style="display: block;">
                <button type="button" class="close" data-hide="alert" aria-label="Close"><span aria-hidden="true">×</span></button>
                <strong>Felicidades!</strong> Has cargado tu inmueble con éxito, deseas destacarlo? consulta nuestros planes, para obtener más visitas a tu anuncio.
</div>
-->
<ol style="" class="pay-step-bar">
    ...
    if ( !is_user_logged_in() ) {
        ...
        <?php } else if ($enable_paid_submission == 'per_listing') {
            echo '<li class="pay-step-block '.$ac_payment.'"><span>'.esc_html__( "Paymentx", "houzez" ).'</span></li>';
        }
    } else {
        if ($enable_paid_submission == 'membership') {
